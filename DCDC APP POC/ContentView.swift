    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 0) {
                ...
                ScrollView {
                    TopSectionView(matcher: matcher)
                }
...
struct TopSectionView: View {
    ...
    var body: some View {
        VStack {
            // Control Buttons
            ControlButtonsView(isListening: $matcher.isListening, onToggle: toggleListening, showMatches: $showMatches, matcher: matcher)
...
struct ControlButtonsView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            // Big centered round Listen button
            ...
            HStack(spacing: 15) {
                ...
                Button(action: { matcher.isCycleEnabled.toggle() }) {
                    HStack(spacing: 4) {
                        Image(systemName: matcher.isCycleEnabled ? "repeat.circle.fill" : "repeat.circle")
                        Text("Cycle")
                    }
                    .padding(.horizontal, 16)
                    .padding(.vertical, 8)
                    .background(matcher.isCycleEnabled ? Color.green.opacity(0.3) : Color.gray.opacity(0.2))
                    .cornerRadius(8)
                }
            }
        }
